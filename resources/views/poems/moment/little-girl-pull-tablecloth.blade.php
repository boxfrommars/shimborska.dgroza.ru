@extends('layout')

@section('content')
    <h2>Маленькая девочка стаскивает скатерть</h2>

    <div class="poem">
        <p>
            Вот уже год на этом свете,<br/>
            а тут еще не все исследовано<br/>
            и взято под контроль.
        </p>
        <p>
            Теперь опробуются вещи,<br/>
            которые не двигаются сами.
        </p>
        <p>
            Нужно им в этом помочь,<br/>
            подвинуть, подтолкнуть,<br/>
            брать с места и переносить.
        </p>
        <p>
            Не все хотят, например, шкаф, буфет,<br/>
            неподдающиеся стены, стол.
        </p>
        <p>
            Но скатерть на столе упрямом —<br/>
            если схватиться за края покрепче —<br/>
            согласна ехать.
        </p>
        <p>
            А на скатерти стаканы, ложки, чашка<br/>
            аж трясутся от охоты.
        </p>
        <p>
            Очень интересно,<br/>
            какое они выберут движенье,<br/>
            когда окажутся уж на краю:<br/>
            гулять по потолку?<br/>
            летать ли вокруг лампы?<br/>
            прыгнуть на подоконник, а оттуда на дерево?
        </p>
        <p>
            Мистер Ньютон пока не при чем.<br/>
            Пусть себе смотрит с неба и машет руками.
        </p>
        <p>
            Эта попытка сделана должна быть.<br/>
            И будет сделана.
        </p>
        <p class="foot-note">Перевод Натальи Астафьевой</p>
    </div>

    <div class="poem">
        <h3>Mała dziewczynka ściąga obrus</h3>

        <p>
            Od ponad roku jest się na tym świecie,<br/>
            a na tym świecie nie wszystko zbadane<br/>
            i wzięte pod kontrolę.
        </p>
        <p>
            Teraz w próbach są rzeczy,<br/>
            które same nie mogą się ruszać.
        </p>
        <p>
            Trzeba im w tym pomagać,<br/>
            przesuwać, popychać,<br/>
            brać z miejsca i przenosić.
        </p>
        <p>
            Nie każde tego chcą, na przykład szafa,<br/>
            kredens, nieustępliwe ściany, stół.
        </p>
        <p>
            Ale już obrus na upartym stole<br/>
            — jeżeli dobrze chwycony za brzegi —<br/>
            objawia chęć do jazdy.
        </p>
        <p>
            A na obrusie szklanki, talerzyki,<br/>
            dzbanuszek z mlekiem, łyżeczki, miseczka<br/>
            aż trzęsą się z ochoty.
        </p>
        <p>
            Bardzo ciekawe,<br/>
            jaki ruch wybiorą,
        </p>
        <p>
            kiedy się już zachwieją na krawędzi:<br/>
            wędrówkę po suficie?<br/>
            lot dokoła lampy?<br/>
            skok na parapet okna a stamtąd na drzewo?
        </p>
        <p>
            Pan Newton nie ma jeszcze nic do tego.<br/>
            Niech sobie patrzy z nieba i wymachuje rękami.
        </p>
        <p>
            Ta próba dokonana być musi.<br/>
            I będzie.
        </p>
    </div>

    <div class="poem">
        <h3>Маленькая девочка потянула скатерть</h3>

        <p>
            Она на этом свете больше года,<br/>
            а на нем не всё пока разведано<br/>
            и не всё под присмотром.
        </p>
        <p>
            Оказывается, некоторые вещи<br/>
            сами не шевелятся.
        </p>
        <p>
            Им надо помочь,<br/>
            сдвинуть, подтолкнуть,<br/>
            взять, где стоят, и перенести.
        </p>
        <p>
            Не каждая на это согласна, скажем, кровать,<br/>
            комод, своенравные стены, стол.
        </p>
        <p>
            А вот скатерть на упрямом столе<br/>
            — если потянуть —<br/>
            желание поехать обнаруживает.
        </p>
        <p>
            Стаканам на ней, тарелкам,<br/>
            молочнику с молоком, блюдцу<br/>
            ужасно охота проехаться.
        </p>
        <p>
            Интересно,<br/>
            что им придет в голову,
        </p>
        <p>
            когда качнутся на краю:<br/>
            гулять по потолку?<br/>
            летать вокруг лампы?<br/>
            прыгнуть на подоконник, а с него на дерево?
        </p>
        <p>
            Мистер Ньютон покамест ни при чем.<br/>
            Пускай глядит с небес и размахивает руками.
        </p>
        <p>
            Этому опыту суждено быть.<br/>
            И он будет.
        </p>
        <p class="foot-note">Перевод Асара Эппеля</p>
    </div>
@endsection

@section('notes')
@endsection

@section('images')
@endsection

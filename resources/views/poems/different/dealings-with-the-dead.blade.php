@extends('layout')

@section('content')
    <h2>Сношения с умершими</h2>

    <div class="poem">
        <p>
            При каких обстоятельствах снятся тебе умершие?<br/>
            Как часто думаешь о них ты перед сном?<br/>
            Кто первым появляется?<br/>
            Всегда ли тот же самый?<br/>
            Имя? Фамилия? Кладбище? Дата смерти?<br/>
            На что ссылаются?<br/>
            На старое знакомство? Родство? Отечество?<br/>
            Говорят ли, откуда пришли?<br/>
            И кто стоит за ними?<br/>
            И снятся ли еще кому одновременно?
        </p>
        <p>
            Похожи их лица на их фото?<br/>
            Состарились ли за истекший срок?<br/>
            Бодрые? Изможденные?<br/>
            Убитые или удалось оправиться от ран?<br/>
            Помнят ли своих убийц?
        </p>
        <p>
            Что у них в руках — опиши предметы?<br/>
            Истлевшие? Заржавевшие? Сгнившие? Обгоревшие?<br/>
            Глаза их? — устрашают? вопрошают? И что?<br/>
            О чем беседуете? О погоде только?<br/>
            О птичках? мотыльках? цветочках?
        </p>
        <p>
            Как часто ставят трудные вопросы?<br/>
            А ты что отвечаешь?<br/>
            Вместо того, чтоб взять и промолчать?<br/>
            Уклончиво переменить тему сна?<br/>
            Проснуться вовремя?
        </p>
        <p class="foot-note">Перевод Асара Эппеля</p>
    </div>
    <div class="poem" lang="pl">
        <h3>Konszachty z umarłymi</h3>
        <p>
            W jakich okolicznościach śnią ci się umarli?<br/>
            Czy często myślisz o nich przed zaśnięciem?<br/>
            Kto pojawia się pierwszy?<br/>
            Czy zawsze ten sam?<br/>
            Imię? Nazwisko? Cmentarz? Data śmierci?<br/>
            Na co się powołują?<br/>
            Na dawną znajomość? Pokrewieństwo? Ojczyznę?<br/>
            Czy mówią, skąd przychodzą?<br/>
            I kto za nimi stoi?<br/>
            I komu oprócz ciebie śnią się jeszcze?
        </p>
        <p>
            Ich twarze czy podobne do ich fotografii?<br/>
            Czy postarzały się z upływem lat?<br/>
            Czerstwe? Mizerne?<br/>
            Zabici czy zdążyli wylizać się z ran?<br/>
            Czy pamiętają ciągle, kto ich zabił?
        </p>
        <p>
            Co mają w rękach — opisz te przedmioty.<br/>
            Zbutwiałe? Zardzewiałe? Zwęglone? Spróchniałe?<br/>
            Co mają w oczach — groźbę? prośbę? Jaką?<br/>
            Czy tylko o pogodzie z sobą rozmawiacie?<br/>
            O ptaszkach? Kwiatkach? Motylkach?
        </p>
        <p>
            Z ich strony żadnych kłopotliwych pytań?<br/>
            A ty co wtedy odpowiadasz im?<br/>
            Zamiast przezornie milczeć?<br/>
            Wymijająco zmienić temat snu?<br/>
            Zbudzić się w porę?
        </p>
    </div>
@endsection

@section('notes')
@endsection

@section('images')
@endsection
